...
export default function Layout() {
		...
		return (
			<div className="absolute right-0 mt-2 w-48 rounded-lg shadow-lg bg-white border border-gray-200 transition-all">
				<div className="px-4 py-2 border-b border-gray-200">
					<p className="text-sm font-medium text-gray-700">Olá, {userData?.name}</p>
				</div>
    ...
    <Link to="/" className="text-3xl font-bold text-gray-800 tracking-tight">
        <span className="text-green-600">Code</span><span className="text-gray-700">Sphere</span>
    </Link>
</div>
				...
				<div className="flex items-center gap-4">
					...
					<div className="relative">
						...
						>
							<p className="font-medium text-xl text-white">{userData?.name[0]}</p>
						</div>
						{listOpen && <List />}
					</div>
				</div>
			...
			<main className="flex flex-row w-full">
				<nav className="w-1/6 h-full bg-green-100 border-r-2 border-gray-300 fixed top-[90px] flex flex-col justify-between">
					<ul className="space-y-4 p-4">
						...
						<li>
							<span className="block text-lg font-medium text-gray-500 cursor-not-allowed hover:bg-gray-200 transition-colors duration-300 ease-in-out rounded p-2">
								Minhas Questions
							</span>
						</li>
						<li>
